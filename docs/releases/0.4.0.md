...
- Hardened lifetime management so `sqlite::result` can outlive its creating `sqlite::query`, and
  connection-pool shared aliases no longer call back into a destroyed pool (`963e898`, `abd735b`).
...
- `command::clear()` now clears retained SQLite parameter bindings, and `result::get_column_decltype()`
  returns an empty string when SQLite reports no declared type for computed expressions (`86e399c`).
